...
public class ExtractArticlesExample {
	...
	// get all URLs that start with "https://www.javatpoint.com/" and add them to
	// the set
		...
		if (urlLinks.size() != 50 && !urlLinks.contains(URL) && (depth < MAX_DEPTH)
			...
			try {
				...
				for (Element ele : availableLinksOnPage) {
					if (ele.attr("abs:href").startsWith("http://www.javatpoint.com")
							|| ele.attr("abs:href").startsWith("https://www.javatpoint.com")) {
